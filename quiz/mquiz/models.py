...
class Quizes():
    ...
    def __init__(self,title,current_user,description, questions, start,end,solvers,duration):
        ...
        quizes.insert_one({'_id':title, 'maker':current_user,'description' :description,'questions':questions,'start':start,'end':end,'solvers':solvers,'duration':duration})
    ...
    def update(title,description,questions,duration,solvers):
        qzs = quizes.find_one({'_id' : title})
        quizes.update_one({'_id':title},{ '$set' : { 'description':description,'questions':questions,'duration':duration,'solvers':solvers} })
